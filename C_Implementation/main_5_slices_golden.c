    ...
    for(t=0;t<T1;++t){
        for(f=0; f<F; ++f){
            ...
            for(k=0;k<Ksz;++k){
                ...
                for(e=0;e<E;++e) acc += xrow[e] * wf[e*F];
            }
            Y[t*F + f] = acc;
        }
    }
...
static inline void bn_time_channel(float *Y,int Tlen,int C,
    ...
    for(t=0;t<Tlen;++t){
        float *yt = Y + t*C;
        for(c=0;c<C;++c){
            nrm = (yt[c]-mean[c]) / sqrtf(var[c]+eps);
...
static inline void avgpool1d_P(const float *Z,int Tlen,int C,int Psz,float *U){
    ...
    for(u=0;u<Tout;++u){
        for(c=0;c<C;++c){
            acc=0.0f;
            for(p=0;p<Psz;++p) acc += Z[(u*Psz + p)*C + c];
...
{
    int g_int;
    float xv, hv, i, o, f, g, c, h;
    for(int j=0;j<H;++j){ h_last[j]=0.0f; c_last[j]=0.0f; }
    float z[4*H];

    for(int t=0;t<Tlen;++t){
        // z = b
        for(g_int=0; g_int<4*H; ++g_int) z[g_int] = b_ifog[g_int];
        ...
        for(int d=0; d<D; ++d){
            ...
            for(g_int=0; g_int<4*H; ++g_int) z[g_int] += xv * Wd[g_int];
        }
        // recurrent part
        for(int hp=0; hp<H; ++hp){
            hv = h_last[hp];
            const float *Rh = R_ifog + hp*(4*H);
            for(g_int=0; g_int<4*H; ++g_int) z[g_int] += hv * Rh[g_int];
        }
        // gates
        for(int j=0;j<H;++j){
            i = 1.f/(1.f+expf(-z[0*H + j]));
            f = 1.f/(1.f+expf(-z[1*H + j]));
            o = 1.f/(1.f+expf(-z[2*H + j]));
            g = tanhf_fast(z[3*H + j]);
            c = f * c_last[j] + i * g;
            h = o * tanhf_fast(c);
    ...
    const float *W,const float *b,int Out,float *y)
{
    ...
    for(j=0;j<Out;++j){
        acc = b? b[j] : 0.0f;
        for(i=0;i<In;++i) acc += x[i]*W[i*Out + j];
        y[j]=acc;
    }
}
...
static void print_vec(const char* name,const float* v,int n) {
    int i;
    printf("%s = [", name);
    for(i=0;i<n;++i) printf("%s%.7e", i?", ":"", v[i]);
    printf("]\n");
}
...
static void run_slice_i(const char* base_dir, int i, float out_tanh[H]) {
    ...
    int tokens[T];
    int i_ins, t, j;
    for(i_ins = 0; i_ins < T; i_ins++) { tokens[i_ins] = 0; }
    path_join3(path,sizeof path, base_dir,sdir,"tokens.bin");
    FILE *ft = fopen(path,"rb");
    if(ft){
        int got = fread(tokens,sizeof(int),T,ft); fclose(ft);
        if(got != (int)T){ fprintf(stderr,"tokens.bin len != %d for %s\n", T, sdir); exit(1); }
    }else{
        for(t=0;t<T;++t) tokens[t] = t % VOCAB_SIZE;
    }
    ...
    for(i_ins = 0; i_ins < T*E; i_ins++) { X0[i_ins] = 0.0f; }
    ...
    float Y[T1*F];
    for(i_ins = 0; i_ins < T1*F; i_ins++) { Y[i_ins] = 0.0f; }
    ...
    float U[T2*F];
    for(i_ins = 0; i_ins < T2*F; i_ins++) { U[i_ins] = 0.0f; }
    ...
    for(j=0;j<H;++j) out_tanh[j] = tanhf_fast(h_last[j]);
...
int main() { // int argc,char** argv
    ...
    const char* WROOT = "/home/gkapakos/Desktop/ECE/10th_Semester/Architecture_of_Parallel_Systems/Project/BranchPredictionAI/Notebooks/weights_out/multi/"; //argv[1];
